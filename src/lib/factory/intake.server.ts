...
export const submitFactoryIntake = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (!baseUrl || !intakeToken) {
      return failure("not_configured", "Factory intake is not connected in this environment.");
    }
    ...
    const cfClientSecret = (process.env.WARLOCK_CF_ACCESS_CLIENT_SECRET ?? "").trim();
    ...
    try {
      ...
      return failure("factory_unavailable", "Factory gateway is currently unavailable.");
    }

    if (response.status === 401 || response.status === 403) {
      return failure("gateway_auth_failed", "Factory gateway rejected the website connection.");
    }
    if (response.status === 409) {
      return failure("request_conflict", "This build request ID was already used for different content.");
    }
    if (!response.ok) {
      return failure("factory_unavailable", "Factory did not accept the build request.");
    }
    ...
    try {
      ...
      return failure("invalid_factory_response", "Factory returned an unverified intake response.");
    }
  });
